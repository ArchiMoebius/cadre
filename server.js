...
var options = {
  ...
  port: Number(argv.p) || Number(process.env.SOCKETCLUSTER_PORT) || 443,
  ...
  killMasterOnSignal: false,// If using nodemon, set this to true, and make sure that environment is 'dev'.
  environment: environment,
  authKey: null,//TODO: why won't public / private keypair work?...fix it!
  //authPrivateKey: "/etc/pki/tls/private/server.pem",
  //authPublicKey: "/etc/pki/tls/certs/server.pem",
  authDefaultExpiry: 86400,
  authAlgorithm: 'HS256',
  protocol: 'https',
  protocolOptions: {
    key: fs.readFileSync('/etc/pki/tls/private/server.pem', 'utf8'),
    cert: fs.readFileSync('/etc/pki/tls/certs/server.pem', 'utf8'),
    dhparam: fs.readFileSync('/etc/pki/tls/private/dhparam.pem', 'utf8'),
...
var start = function () {
  ...
  if (socketCluster.options.environment === 'dev') {
    ...
    scHotReboot.attach(socketCluster, {
      cwd: __dirname,
      ignored: ['public', 'backend.db-journal', 'backend.db', 'node_modules', 'README.md', 'Dockerfile', 'server.js', 'broker.js', /[\/\\]\./, '*.log']
    });
  }
};
